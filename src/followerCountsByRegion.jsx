  ...
  const cleanData = (responce) => {
    ...
    resp.map((dailysplit) => {
      for (var i in Object.entries(
        dailysplit["followers_split"]["followerCountsByRegion"]
      )) {
        for (var j in dailysplit["followers_split"]["followerCountsByRegion"][
          i
        ]) {
          if (j === "regionText") {
            sentimentObject["regionText"] =
  ...
  return (
    <>
      {data && (
        <>
          <DownloadExcel
            title="followerCountsByRegion"
            data={data}
            fileName="followerCountsByRegion"
            buttonLabel="followerCountsByRegion"
