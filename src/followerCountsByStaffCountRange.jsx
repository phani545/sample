  ...
  const cleanData = (responce) => {
    ...
    resp.map((dailysplit) => {
      for (var i in Object.entries(
        dailysplit["followers_split"]["followerCountsByStaffCountRange"]
      )) {
        for (var j in dailysplit["followers_split"][
          "followerCountsByStaffCountRange"
        ][i]) {
          if (j === "staffCountRange") {
            ...
            for (var k in dailysplit["followers_split"][
              ...
              if (k === "organicFollowerCount") {
                sentimentObject["followerCounts"] =
                  dailysplit["followers_split"][
                    "followerCountsByStaffCountRange"
                  ][i]["followerCounts"]["organicFollowerCount"];
              }
            }
          }
        }
        //console.log("followerCountsByStaffCountRange", sentimentObject);
  ...
  return (
    <>
      {data && (
        <>
          <DownloadExcel
            title="followerCountsByStaffCountRange"
            data={data}
            fileName="followerCountsByStaffCountRange"
            buttonLabel="followerCountsByStaffCountRange"
...
export default CountsByStaffCountRange;
